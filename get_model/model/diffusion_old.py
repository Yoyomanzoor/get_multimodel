...
# -----------------------------------------------------------------------------
# Finetune Model: GETRegionFinetuneDiffusion
# -----------------------------------------------------------------------------

@dataclass
class GETRegionFinetuneDiffusionConfig(BaseGETModelConfig):
    """
    Configuration for region-based diffusion finetune model.
    Matches GETRegionFinetune structure but uses DiT encoder with timestep conditioning.
    """
    num_regions: int = 900
    num_motif: int = 283
    embed_dim: int = 768
    ...
    num_heads: int = 12
    dropout: float = 0.1
    output_dim: int = 2  # Expression prediction output
    flash_attn: bool = False
    pool_method: str = 'mean'
    ...
    # Expression head config
    head_exp: ExpressionHeadConfig = field(default_factory=ExpressionHeadConfig)
    use_atac: bool = False
    
    # Diffusion config (needed for DiT encoder timestep conditioning)
    diffusion: dict = field(default_factory=lambda: {
        'num_timesteps': 1000,
        'beta_start': 0.0001,
        'beta_end': 0.02,
...
class GETRegionFinetuneDiffusion(BaseGETModel):
    """
    Region-based Diffusion Finetune Model for expression prediction.
    
    Architecture mirrors GETRegionFinetune but uses DiT encoder:
    - Input: region_motif (B, N, num_motif)
    - RegionEmbed -> DiT Encoder with timestep conditioning -> ExpressionHead
    
    Training: Expression prediction with timestep conditioning (for consistency with pretrained DiT).
    During training, timesteps are sampled; during inference, t=0 is used.
    """
    def __init__(self, cfg: GETRegionFinetuneDiffusionConfig):
        super().__init__(cfg)
        
        # 1. Region Embedding (same as GETRegionFinetune)
        self.region_embed = RegionEmbed(cfg.region_embed)
        ...
        # 3. Expression head (same as GETRegionFinetune)
        self.head_exp = ExpressionHead(cfg.head_exp)
        
        # 4. CLS token (no mask token for finetuning)
        self.cls_token = nn.Parameter(torch.zeros(1, 1, cfg.embed_dim))
        trunc_normal_(self.cls_token, std=0.02)
        
        # 5. Diffusion schedule (needed for timestep sampling)
        self._setup_diffusion(cfg.diffusion)
    ...
    def _setup_diffusion(self, diff_cfg):
        """Setup noise schedule for diffusion (needed for timestep sampling)."""
        num_timesteps = diff_cfg.get('num_timesteps', 1000)
        beta_start = diff_cfg.get('beta_start', 0.0001)
        beta_end = diff_cfg.get('beta_end', 0.02)
    ...
    def get_input(self, batch, perturb=False):
        """Get input from batch - matches GETRegionFinetune."""
        return {
            'region_motif': batch['region_motif'],
        }

    def forward(self, region_motif, timestep=None):
        """
        Forward pass for expression prediction.
        
        Args:
            region_motif: (B, N, num_motif) - Input motif features
            timestep: Optional (B,) tensor of timesteps. If None, samples during training or uses t=0 during inference.
        
        Returns:
            exp: (B, N, output_dim) - Expression predictions for all regions
        """
        # 1. Embed regions
        x = self.region_embed(region_motif)
        B, N, C = x.shape
        
        # 2. Add CLS token (no masking for finetuning)
        cls_tokens = self.cls_token.expand(B, -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)
        
        # 3. Sample or use timestep
        device = x.device
        if timestep is None:
            if self.training:
                # During training, sample random timesteps
                t = torch.randint(0, self.num_timesteps, (B,), device=device).long()
            else:
                # During inference, use t=0 (no noise)
                t = torch.zeros((B,), device=device, dtype=torch.long)
        else:
            t = timestep
        
        # 4. Run through DiT encoder with timestep conditioning
        x = self.encoder(x, t)
        
        # 5. Remove CLS token and predict expression for all regions
        x = x[:, 1:]  # Remove CLS token
    ...
    def before_loss(self, output, batch):
        """Prepare predictions and targets for loss computation - matches GETRegionFinetune."""
        pred = {'exp': output}
        obs = {'exp': batch['exp_label']}
        return pred, obs

    def generate_dummy_data(self):
        """Generate dummy data for testing - matches GETRegionFinetune."""
        B, R, M = 2, 900, 283
        return {
            'region_motif': torch.randn(B, R, M).float().abs(),
        }
